...
function ConversationChat({
  ...
  const handleNewMessageChange: ChangeEventHandler<HTMLInputElement> = (
    event
  ) => {
  ...
  useEffect(() => {
    const lastCommentElement =
      messagesWrapperRef.current?.lastElementChild?.lastElementChild;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {messages.length > 0 && !loading && (
        <div
          className="flex flex-col gap-5 px-[30px] py-5 flex-1 overflow-scroll"
        ...
        >
          {messages.map((message, index) => (
            <MessageItem
              message={message}
              style={{
                alignSelf: message.sender._id === user._id ? "end" : "start",
              }}
              key={`message-${message.sender._id}-${index}`}
            />
          ))}
        </div>
      )}
